...
# Display dropdown for existing profiles
txt_files = get_txt_files_in_profiles()
options = display_file_names_without_extension(txt_files)
selected_user = st.sidebar.selectbox("Select an existing User Profile:", options)
...
st.sidebar.header("Add New Profile")
new_profile_name = st.sidebar.text_input("Enter Profile Name:")
create_button = st.sidebar.button("Create")

if create_button:
    new_profile_path = os.path.join('Profiles', f'{new_profile_name}.txt')
    
    if os.path.exists(new_profile_path):
        st.sidebar.warning(f"Profile '{new_profile_name}' already exists!")
    else:
        create_profile(new_profile_path)
        st.sidebar.success(f"Profile '{new_profile_name}' created successfully!")
...
def main():
    ...
    if st.button('Submit'):
        ...
        all_messages.append('Duke: %s' % response)
        st.write('\n\Duke:\n\n %s' % response)
